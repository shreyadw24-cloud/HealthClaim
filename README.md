...
│   ├── src/
│   │   ├── content/             # In-page detection, button, overlay, messaging
│   │   ├── background/          # Service worker: API calls, tab audio/screenshot capture, history
│   │   ├── offscreen/           # Offscreen document (audio recording)
│   │   ├── App.tsx               # Popup UI
│   │   └── history-main.tsx      # History page
│   ├── public/manifest.json
│   └── package.json
├── server/                     # Node.js + Express backend
│   ├── src/
│   │   ├── ai/                   # Claim extraction (text/image/audio), classification, Gemini client
│   │   ├── evidence/             # Evidence search, ranking, trusted source list
│   │   ├── db/                   # Supabase client + verification history
...
npm install
```
Configure environment variables in `client/.env` (see below), then:
```bash
npm run dev
```
...
# Service-role key, NOT the anon/public key — this runs server-side only
# and needs full insert/select access to the verifications table.
